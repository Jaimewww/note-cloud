...
public class AuthBean implements Serializable {
    ...
    private String redirectUri = "http://localhost:9080/callback.xhtml";
    ...
    public void procesarCallback() throws IOException {
        ...
        if (!estadoSes.equals(estadoRecibido)) {
            ...
            String access_token = reclamarToken(code, clientID, clientSecret, redirectUri);
            JSONObject userInfo = obtenerUserInfo(access_token);

            Usuario usuario = new Usuario();
            usuario.setGoogleId(userInfo.getString("sub"));
            usuario.setNombre(userInfo.getString("name"));
            usuario.setEmail(userInfo.getString("email"));
            usuario.setImagenUrl(userInfo.getString("picture"));

            usuarioBean.setAccessToken(access_token);
            usuarioBean.setUsuarioLogueado(usuario);
